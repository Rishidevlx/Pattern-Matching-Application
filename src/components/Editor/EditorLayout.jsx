...
const EditorLayout = ({ userData, onLogout }) => {
    ...
    const [codeMap, setCodeMap] = useState(() => {
        if (userData.dbCodeData) {
            try {
                const parsedDbCodeData = JSON.parse(userData.dbCodeData);
                if (parsedDbCodeData && parsedDbCodeData.map) {
                    return parsedDbCodeData.map;
                }
                return parsedDbCodeData; // Fallback for old flat structures
            } catch (e) { }
        }
        try {
    ...
    useEffect(() => {
        if (userData.lotNo) {
            localStorage.setItem(`qmaze_lang_${userData.lotNo}`, language);
        }
    }, [language, userData]);
    ...
    // --- PERSISTENCE EFFECT & DB RESTORE LOGIC (Absolute Timer) ---
    useEffect(() => {
        // If DB has a language, enforce it and lock it tight.
        if (userData.dbCodeData) {
            try {
                const parsedDbCodeData = JSON.parse(userData.dbCodeData);
                if (parsedDbCodeData.language) {
                    setLanguage(parsedDbCodeData.language);
                    localStorage.setItem(`qmaze_lang_${userData.lotNo}`, parsedDbCodeData.language);
                }
            } catch (err) { }
        }

        // Initial Mount Disqualify check
        if (initialDuration > 0 && userData.dbStartTime) {
            const startTimestamp = new Date(userData.dbStartTime).getTime();
            const timeElapsedAbsolute = Date.now() - startTimestamp;
            const remaining = Math.max(0, initialDuration - timeElapsedAbsolute);
    ...
    }, [initialDuration, userData.dbStartTime, userData.dbStatus, userData.dbCodeData]);
    ...
    useEffect(() => {
        const fetchPatterns = async () => {
            try {
                ...
                // Set level based on patterns_completed
                if (data.length > 0) {
                    const completedCount = userData.dbPatternsCompleted || 0;
                    // Prevent index out of bounds if they completed all
                    const targetIndex = Math.min(completedCount, data.length - 1);
    ...
    // Custom Session Start Time to prevent Timer Jump
    // Timer Logic & Disqualification (Absolute Hackathon Timer)
    useEffect(() => {
        let timer;
        ...
        if (isSessionActive && metrics.time > 0 && !isSuccess && !isDisqualified) {
            timer = setInterval(() => {
                setMetrics(prev => {
                    // Always calculate from EXACT DB START TIME
                    if (userData.dbStartTime && initialDuration > 0) {
                        const startTimestamp = new Date(userData.dbStartTime).getTime();
                        const timeElapsedAbsolute = Date.now() - startTimestamp;
                        const newTime = Math.max(0, initialDuration - timeElapsedAbsolute);
                    ...
                    // Fallback to simple decrement if session just started and state hasn't caught up
    ...
    const handleStartSession = async () => {
        ...
        try {
            ...
            if (data.success && data.start_time) {
                // The timer effect will automatically pick this up and calculate Elapsed Time
                // We manually inject it into userData locally right now to trigger the effect
                ...
                setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), text: '> ERROR: FAILED TO START SESSION.', type: 'error' }]);
            }
        } catch (err) {
            console.error("Backend Sync Error:", err);
            setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), text: `> NETWORK ERROR: ${err.message}`, type: 'error' }]);
        }
    };
    ...
    const triggerSync = async (overrides = {}) => {
        ...
        try {
            ...
            // Calculate absolute time taken based on the exact offset since the session started or resumed
            const timeTaken = currentState.initialDuration - currentState.metrics.time;

            // Bundle the language inside the codeMap payload for DB syncing
            const dbCodeDataPayload = {
                language: language,
                map: cleanMap
            };
            ...
            await fetch(`${import.meta.env.VITE_API_URL}/api/update-progress`, {
                ...
                body: JSON.stringify({
                    lotNumber: userData.lotNo,
                    code: activeCode || '',
                    codeMap: dbCodeDataPayload, // Encapsulating map + language together
                    totalTime: timeTaken,
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            if (result.run) {
                ...
                // Piston Error handling (Backend sent error inside stderr)
                if (result.message && !result.run.stdout) {
                    displayOutput = `>>> EXECUTION ERROR: ${result.message} <<<`;
                    logType = 'error';
                }
                    ...
                    <div style={{ flex: 1, position: 'relative', borderRight: '1px solid #333', height: '100%', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
                            <CodeEditor
                                ...
                                language={language}
                                readOnly={!isSessionActive || isSuccess || isDisqualified || completedPatterns.includes(currentPattern?.id)}
                            ...
                            {isDisqualified && (
                                <div style={{
                                    ...
                                    }}>YOU ARE DISQUALIFIED!</h1>
                                    <h2 style={{ color: '#fff', fontFamily: 'monospace', fontSize: '2rem' }}>TIME'S UP</h2>

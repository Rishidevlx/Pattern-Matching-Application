...
const Dashboard = () => {
    ...
    const [config, setConfig] = useState({ COLLEGE_NAME: 'QMAZE', DEPT_NAME: 'COMPETITION' });
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                // Extract College and Dept if they exist
                if (settingsRes.data) {
                    setConfig({
                        COLLEGE_NAME: settingsRes.data.COLLEGE_NAME || 'QMAZE',
                        DEPT_NAME: settingsRes.data.DEPT_NAME || 'COMPETITION'
                    });
                }
            } catch (err) {
    ...
    const generatePDF = () => {
        ...
        // --- HEADER DESIGN ---
        doc.setFillColor(0, 20, 0); // Dark Green background for header
        doc.rect(0, 0, 210, 40, 'F');

        doc.setTextColor(0, 255, 65); // Neon Green text
        doc.setFontSize(22);
        doc.setFont('helvetica', 'bold');
        doc.text(config.COLLEGE_NAME.toUpperCase(), 105, 15, { align: 'center' });

        doc.setTextColor(255, 255, 255);
        doc.setFontSize(14);
        doc.text(config.DEPT_NAME.toUpperCase(), 105, 25, { align: 'center' });
        ...
        autoTable(doc, {
            startY: 45,
            head: [['RANK', 'LOT NO', 'PATTERNS', 'LOOPS', 'TIME', 'LOC', 'WARN', 'STATUS']],
            body: tableData,
            theme: 'striped',
            headStyles: { fillColor: [0, 60, 0], textColor: [255, 255, 255], fontStyle: 'bold' },
            styles: { fontSize: 9, cellPadding: 3 },
            alternateRowStyles: { fillColor: [240, 255, 240] }
        });

        doc.save(`${config.COLLEGE_NAME}_Leaderboard.pdf`);
    };
                    ...
                    <tbody>
                        {users.map((user, index) => (
                            ...
                            >
                                ...
                                <td>
                                    <div style={{ fontWeight: 'bold', color: '#fff', fontSize: '1.2rem' }}>{user.lot_number}</div>
                                </td>
                                <td style={{ textAlign: 'center', fontWeight: 'bold', color: '#00ffff' }}>{user.patterns_completed || 0}</td>
                                <td style={{ textAlign: 'center', fontWeight: 'bold', color: '#ffcc00' }}>{user.no_of_loops || 0}</td>
                                <td style={{ fontFamily: 'monospace', fontSize: '1.1rem', color: '#00ff41', textAlign: 'center' }}>{formatTime(user.total_time)}</td>
                                <td style={{ textAlign: 'center', fontWeight: 'bold' }}>{user.lines_of_code}</td>
